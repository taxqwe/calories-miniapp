document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupBottomNavigation() {
    ...
    // Переходы по нижней навигации — через location.replace, чтобы webview не
    // копил историю вкладок. Тогда системный «назад» на верхнем уровне закрывает
    // мини-апп, а не листает вкладки. ?lang= уже проставлен в href выше.
  ...
  // На верхнеуровневой вкладке прячем Telegram BackButton, чтобы работало
  // нативное закрытие мини-аппа по «назад».
  if (tg && tg.BackButton) {
